...
export default {
  ...
  // 2026-09-12 数据体检页 i18n（短标签；长说明回退英文）
  'Data health check': '🩺 Verificação de dados',
  ...
  '{files} files · {lines} memos · {issues} issues': '{files} arquivos · {lines} memos · {issues} problemas',
  ' (incl. {n} legacy-format files)': ' (incl. {n} arquivos em formato antigo)',
  ' ({n} auto-fixable)': ' ({n} corrigíveis)',
  ...
  'Migrating…': 'Migrando…',
  'Convert the legacy rows of this file into card blocks (automatic backup). Your memos show up in the feed again afterwards.': 'Converte as linhas em formato antigo deste arquivo em blocos-cartão (backup automático). Seus memos voltam a aparecer na lista depois disso.',
  '{rule} — fix to:': '{rule} — corrigir para:',
  'Fix this line': 'Corrigir esta linha',
  View: 'Ver',
  Ignore: 'Ignorar',
  'Fix all': 'Corrigir tudo',
  'Scan failed: ': 'Falha na verificação: ',
  'Migration failed: ': 'Falha na migração: ',
  'no auto-fixable issues': 'nenhum problema corrigível automaticamente',
  'cannot auto-fix further — the remaining issues need manual work or a migration': 'não é possível continuar a correção automática — os problemas restantes exigem trabalho manual ou migração',
  ...
  'Migration done: {n} entries converted': 'Migração concluída: {n} entradas convertidas',
  'Migrated all: {files} files · {n} entries converted': 'Migração completa: {files} arquivos · {n} entradas convertidas',
  ', {n} cross-day comments moved to their daily notes': ', {n} comentários entre dias movidos para as respectivas notas diárias',
  ', {n} deleted comments dropped': ', {n} comentários excluídos descartados',
  ', {n} entries kept as-is (could not be mapped)': ', {n} entradas mantidas como estavam (não mapeáveis)',
  ', failed: {list}': ', falhas: {list}',
  '. Backups are in .rememo-backup/migrate-*. Your old memos are now card blocks.': '. Os backups estão em .rememo-backup/migrate-*. Seus memos antigos agora são blocos-cartão.',
  '. Backups are in .rememo-backup/migrate-*.': '. Os backups estão em .rememo-backup/migrate-*.',
  'Nothing to migrate ({n} lines lack a time and need manual work).': 'Nada a migrar ({n} linhas sem horário, precisam de trabalho manual).',
  'This file has no legacy-format lines — no migration needed.': 'Este arquivo não tem linhas em formato antigo — não é necessária migração.',
  'Legacy <br> line breaks': 'Quebras de linha <br> antigas',
  'Duplicate ^id': '^id duplicado',
  'Legacy format row': 'Linha em formato antigo',
  'Legacy 14-digit timestamp': 'Data/hora antiga de 14 dígitos',
  'Missing ^id': '^id ausente',
  'Your old memos are still here': 'Seus memos antigos continuam aqui',
  'This vault contains {n} memo lines in the old Memos format, which Rememo does not render yet. Nothing is lost — run the data health check to convert them into card blocks.': 'Este cofre contém {n} linhas de memos no formato antigo do Memos, que o Rememo ainda não exibe. Nada foi perdido — rode a verificação de dados para convertê-las em blocos-cartão.',
  'Open data health check': 'Abrir verificação de dados',
};
