...
> Domain glossary + handover doc for the Obsidian plugin **Rememo** (`rememo`, fork of Quorafind's Obsidian-Memos). Kept in sync as work proceeds. **Last updated: 2026-09-05.**
...
- **P1b 视觉二/三轮反馈已落地（2026-09-05，commits 3dfa3c0 / 7caf6c4，已 push），owner 目视通过（2026-09-05 "看起来差不多了"）**: 角标改为**左上 46×12px 半透明短斜带**（.memo-task-corner：accent 32%/完成置灰 14%，右端 45° 斜切、无图形、只盖顶部 padding 区不碰时间行、不做内容让位；回收站同款）；勾选框**放回时间文本右侧**；三点菜单容器 `width:max-content + min-width:112px`（主列表/回收站两处，长文案不再溢出），切换任务文案定稿 **「设为任务卡 / 取消任务卡」**（key TURN INTO TASK/TURN INTO MEMO）。**遗留目视项：2026-09-04 批次的次级界面 token 化**（弹窗/回收站/设置/热力图/标签/查询/侧栏浅深+换 accent）——owner 2026-09-05 确认"用着没啥大问题"，记为已通过。
- **P2 cm6 输入内核（2026-09-05，commits b98515b + bbb87c7，已 push，但运行期键盘/联想未生效——**待排查交接，见下）**: rta → 自打包 CodeMirror 6（state/view/commands/autocomplete + format/keys/highlight/suggest 四模块）；列表续行、原生 undo、编辑器缓存改造完成。**owner 复测结论（2026-09-05）**：Enter 续行生效；但 ① Ctrl/Cmd+Enter 发送与设置 EnterToSend 两种模式的 Ctrl+Enter 均无效 ② Mod-B/I/E 格式快捷键无效 ③ `#`/`[[` 联想弹层均不出现（suggest.ts 的 # token 错位 bug 已修但未及复验）④ 输入框聚焦时出现"虚线框"（聚焦 outline，样式防御已加 outline:none 未复验）⑤ 输入区字体/换行观感待调。owner 决定暂停，问题记录交接。

**P2 交接排查线索（下一位先读这里）**
- **症状模式**：无修饰 Enter（续行 keymap）生效；所有 **Mod 组合键**（Ctrl+Enter 发送、Mod-B/I/E）不生效，连 EditorView.domEventHandlers 层的 Ctrl+Enter 也不触发；autocomplete 弹层（tooltip）不出现。基础编辑/发送（点击按钮）/内容缓存都正常 → EditorState 与 updateListener 管道是通的。
- **最可疑方向**：Mod 键事件在抵达 cm contentDOM 前被 Obsidian 全局键盘处理吞掉（capture 阶段）；或 view 焦点未真正落在我们 contentDOM（但打字正常…）。建议排查顺序：① 浏览器控制台看是否有 cm logException/其它报错；② 临时在 extensions 里加 `EditorView.domEventHandlers({keydown: (e)=>{console.log('cm-keydown', e.key, e.ctrlKey); return false;}})` 确认事件是否到达 cm（现在 Ctrl+Enter 的 dom handler 不触发 → 很可能根本没到）；③ 若确认被吞，在 window/document 的 capture 阶段自行注册监听（Obsidian workspace keymap 也在此层，需比较注册顺序）处理 Mod+Enter/Mod+B 等；④ autocomplete 弹层若仍不出现，检查 tooltip 渲染（cm-tooltip 类样式在 editor.less 已配）与 console 报错。
- **已知 bug（已修未复验）**：suggest.ts tag token 曾错位 hash+1→hash（headless 测试覆盖：'说 #工' pos4 → from=3）。keys.ts 的 Ctrl+Enter 现在同时存在于 keymap（兜底）与 domEventHandlers（主路径）；EnterToSend 设置走 MemoEditor→Editor props ref（运行时生效不需重建）。
- **样式遗留**：聚焦虚线框（focus outline，.cm-content outline:none 已加防御未复验）；输入区字体/换行观感待 owner 提供截图细节。
- 文件：src/editor/{keys,suggest,highlight,format}.ts、src/components/Editor/Editor.tsx（cm6 host）、src/less/editor.less（.cm-host/.cm-tooltip 主题）。依赖：@codemirror/{state,view,commands,autocomplete}；已卸 tiny-undo/rta。main.js 1346 KiB（gzip 317）。
...
0. **P2 cm6 输入内核运行期问题（下一块，交接线索见 Completed 区 P2 条目下方）** — 代码已提交（b98515b/bbb87c7），Enter 续行正常；Ctrl+Enter 发送/Mod 格式快捷键/联想弹层不生效、聚焦虚线框与输入字体观感未复验。owner 2026-09-05 决定暂停记录，下一位按排查线索优先处理（怀疑 Obsidian 全局 capture 吞 Mod 键等）。
1. **P1b 目视 + 微调** — ✅ 全部完成（owner 目视通过 2026-09-05；次级界面 token 化目视同日口头确认无碍）。
