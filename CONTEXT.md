...
- 插件 Rememo（id `rememo`）。**命名事实**：开发期 id 曾是 `obsidian-memos-plus`，但对外的名字从来只有 Memos——不存在「Memos Plus」这个名字，对外文案别写。仓库：`L:\Files\ObsidianDevVault\.obsidian\plugins\obsidian-rememo`，分支 `dev`，pnpm+vite，`pnpm build` 出 main.js/styles.css 随提交附。`L:\Files\md-note-repo` 是正式库（322 日记，2026-09-05 盘点）**勿碰**。HEAD 见 `git log -1`。
...
- 开发库界面语言改英文后**不必改回**（owner 无异议）。发布线：1.0.0 已完成（快进 `main` / tag / Release 三件套）；**1.0.0 审查未通过 → 整改后发 1.0.1 重新提交**（见下）。

**待开发侧**：批 2（审查 Warning 档，见上）留到过审后再清；其余无——README 中英两版、截图、发布线均已就绪。

**开发库 MCP**：Local REST API 在开发库实例跑 `27125`（正式库实例仍占 27123）——`mcp__obsidian__*` 操作的是**开发库**（见 memory `obsidian-mcp-targets-real-vault`）。

**社区审查整改（2026-09-11，批 1 已完成并 owner 目视通过）**
...
- **发布线（1.0.1，2026-09-11 全部完成）**：①提交本批 → ②`main` 快进（`bc2c711`）→ ③推 tag **1.0.1** → 工作流自动建 Release（三件套 + **构建证明**）✅ → ④owner 到 community.obsidian.md 提交，**审查通过、dashboard 显示已发布** ✅。
- **上架后收尾（已完成）**：README 中英两版删掉「待目录审核」说明（`9e7622d`，README 由目录读默认分支 HEAD，**无需发新版本**）；仓库 Issues 已开启。
- **剩**：批 2（审查 Warning 档）择机清。
- **1.0.2（2026-09-12）**：老用户迁移引导批（横幅 + 审计页 i18n + 错误边界）。发版走同一流程：提交 → 快进 `main` → 推 tag `1.0.2`（工作流自动出 Release + 构建证明）。
- **1.0.3（2026-09-13）**：Rememo 内字号设置（`ContentFontSize` 默认 16px/可选跟随全局，只影响插件不改全局笔记）+ 占位词伪元素→真实 DOM 元素（正式库自定义 CSS 的 ::before 全局重置会杀没占位内容）。发版：提交 → 快进 `main` → 推 tag `1.0.3`（工作流 ✅ Release + 构建证明）。
- **1.0.4（2026-09-13）**：输入框字号跟随修复（内核 DOM 包装层选择器 + 行高随字号）+ Ko-fi 捐赠入口（manifest `fundingUrl` + 设置页常量）。发版同流程：提交 → 快进 `main` → 推 tag `1.0.4`（工作流 ✅ Release + 构建证明）。

**发布线（2026-09-11 首次执行，1.0.0）——四步含义与自动化留档**

- **已执行（2026-09-11）**：
  1. ✅ `main` 已快进到 `dev` 同一提交 `1f22756` 并推送（默认分支即 `main`，市场只读它的 HEAD manifest）
  2. ✅ tag `1.0.0` 已推送（与 manifest.version 一致）
  3. ✅ GitHub Release 已建、附三件套：https://github.com/Lynx3x/obsidian-rememo/releases/tag/1.0.0
