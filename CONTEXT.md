...
0. **回收站总开关**（2026-09-06 grill 定案；**2026-09-09 实施，待 owner 目视**）：设置项「启用回收站」默认开——关 = 删除两段确认后**直接永久删**（第二段确认措辞从 CONFIRM！改「永久删除?」，Memo 菜单 + MemoCardDialog title 两处）；侧栏「回收站」入口隐藏；主路由表剔除 `/recycle`（彻底封锁，残留 pathname 落入主页）；开关只控今后删除路径，**旧软删数据保留原样，重开恢复**。
