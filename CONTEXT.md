...
- 数据刷新时机（2026-09-10）：全量重读仅视图首开/重开（onOpen invalidate）、手动刷新（点标题）、设置更新、文件删除时发生；**路由切页不重读**（曾致统计行/热力图随分批加载跳动）；文件改动走 fetchMemosFromFile 增量。
...
0. **统计行切页跳动修复 + 口径统一**（2026-09-10 实施，**待 owner 目视**）：**切页不再全量重读**——MemoList/MemoTrash 挂载查 `memoService.isInitialized` 复用已有数据（跳动根因 = 每次切页分批重读、store 整表替换，统计/热力图/标签随部分集合坐过山车，DAY 跳幅最大）；视图重开 `onOpen` 调 `invalidate()` 强制一次全量（关闭期间 vault 监听随 registerEvent 解绑、会漏外部改动）。口径（owner 拍板）：统计行 MEMO/DAY 与标签收集（updateTagsState）**排除已删**（`!isDeleted`，与主列表可见口径一致）；MEMO 硬编码改 t() 键（五语言值皆 'MEMO'）。热力图与分享图脚注仍含已删（未动，要统一说一声）。
1. **P1.5 迁移正式化**（2026-09-05 dev 库全面测试，owner 已跑 8 样本迁移目视：**结论 OK**——评论行折叠进正文嵌套列表即为现行定案（v1 保真，P3 后不再改）；**正式库启用待排期**）：真实范围 = 正式库 `## Memo` 区内 **407 memo + 68 评论行**（38 文件；274/322 有 memo；`## Tasks` 区与 delete.md 都在处理区外，迁移不碰）。已修 computeScope 标题型 token 空转 bug（e194339；token 行只开门不入区，与读取器语义一致）。dev 库处理区已切 `## Memo`。样本池：`bak/prod-memos-20260905/`。
2. **阶段 G**：formatMemoContent 渲染与图片/标签结构化拆分（小重构）。
3. **F2 小红书导出**（最低优先，可弃）。

## 5. Roadmap 剩余（2026-09-10 全清：视觉组最后一条热力图已由本批消化；新方向再登记）
