...
1. **blockquote 渲染缺口**（2026-09-05 owner 实测；**2026-09-09 修复，待 owner 目视**）：marked.ts `renderMemoContentLines` 补引用行分支——行首 `>` 攒段、支持 `>>` 嵌套（栈式开合）、裸 `>` 段分、同深度多行 `<br>` 软换行并入一 `<p>`；行内增强对引用内容自动生效（粗体/高亮/链接同段落）。样式：6 宿主 + 移动端 + 回收站双宿主补 `blockquote` 规则（`--blockquote-border/background-color` 主题变量、竖线+淡底对齐 Obsidian 原生观感）。**范围边界**：列表行内嵌引用（`- > x`）与代码块内引用不转义（行式引擎无容器树）；以 `>` 开头的字面文本无转义通道。复验样例 `daily/2026-09-09.md`（k2mq4x~c6s8xf 六卡）；旧格式真实卡 `daily/2022-10-19.md` 微博引用形态同验。
