...
## 5. Roadmap / 候选清单（2026-09-10 owner 新登记 8 条，未排期——动手前先 grill 细化）

- FEAT **发送音效**：默认不配；允许用户配一个音效路径（空 = 静音）。
- BUG **发送失败兜底**：发送异常时保留输入框内容（或还原回去）——不能让用户辛苦输入的数据丢失。
- STYLE **工具栏任务切换钮**：切换 普通/任务 时输入框内要有可见的样式变化（让切换"看得见"）。
- BUG **热力图 × 检索器**（待检索器重做时一并处理）：①点某日筛选后，检索条显示当天条件但被缩略显示不全；②经「取消检索规则」退出的路径，日历格高亮不消失。
- BUG **关闭 Memos 视图时**右上角误弹「成功打开 memos」提示（待查 Notice 触发链）。
