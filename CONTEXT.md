...
0. **P1.5 迁移正式化**（2026-09-05 dev 库全面测试，owner 已跑 8 样本迁移目视：**结论 OK**——评论行折叠进正文嵌套列表即为现行定案（v1 保真，P3 后不再改）；**正式库启用待排期**）：真实范围 = 正式库 `## Memo` 区内 **407 memo + 68 评论行**（38 文件；274/322 有 memo；`## Tasks` 区与 delete.md 都在处理区外，迁移不碰）。已修 computeScope 标题型 token 空转 bug（e194339；token 行只开门不入区，与读取器语义一致）。dev 库处理区已切 `## Memo`。样本池：`bak/prod-memos-20260905/`。
