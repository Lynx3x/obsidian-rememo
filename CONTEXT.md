...
| **回收站** | isDeleted 的卡（头行 deletedAt）；恢复=去标记、永久删除=删整块。总开关「启用回收站」（设置）关闭时删除不再进回收站、入口隐藏（见 §4 0）。 |
...
0. **回收站总开关**（2026-09-06 grill 定案；2026-09-09 实施，**owner 目视通过，批闭环**）：设置项「启用回收站」默认开——关 = 删除两段确认后**直接永久删**（第二段确认措辞从 CONFIRM！改「永久删除?」，Memo 菜单 + MemoCardDialog title 两处）；侧栏「回收站」入口隐藏；主路由表剔除 `/recycle`（彻底封锁，残留 pathname 落入主页）；开关只控今后删除路径，**旧软删数据保留原样，重开恢复**。
1. **blockquote 渲染缺口**（2026-09-05 owner 实测；**2026-09-09 修复并目视通过，批闭环**）：marked.ts `renderMemoContentLines` 补引用行分支——行首 `>` 攒段、支持 `>>` 嵌套（栈式开合）、裸 `>` 段分、同深度多行 `<br>` 软换行并入一 `<p>`；行内增强对引用内容自动生效（粗体/高亮/链接同段落）。样式：6 宿主 + 移动端 + 回收站双宿主补 `blockquote` 规则（`--blockquote-border/background-color` 主题变量、竖线+淡底对齐 Obsidian 原生观感；嵌套每层重画竖线、段分 8px 段距，1bb930d 观感收尾）。**范围边界**：列表行内嵌引用（`- > x`）与代码块内引用不转义（行式引擎无容器树）；以 `>` 开头的字面文本无转义通道。复验样例 `daily/2026-09-09.md`（k2mq4x~c6s8xf 六卡）；旧格式真实卡 `daily/2022-10-19.md` 微博引用形态同验。
2. **P1.5 迁移正式化**（2026-09-05 dev 库全面测试，owner 已跑 8 样本迁移目视：**结论 OK**——评论行折叠进正文嵌套列表即为现行定案（v1 保真，P3 后不再改）；**正式库启用待排期**）：真实范围 = 正式库 `## Memo` 区内 **407 memo + 68 评论行**（38 文件；274/322 有 memo；`## Tasks` 区与 delete.md 都在处理区外，迁移不碰）。已修 computeScope 标题型 token 空转 bug（e194339；token 行只开门不入区，与读取器语义一致）。dev 库处理区已切 `## Memo`。样本池：`bak/prod-memos-20260905/`。
3. **阶段 G**：formatMemoContent 渲染与图片/标签结构化拆分（小重构）。
4. **F2 小红书导出**（最低优先，可弃）。
5. **Roadmap 剩余**（§5，需 owner 细化）。

## 5. Roadmap 剩余（2026-09-06 清理后：导航/布局组与 Import/About 已由 ADR-0004 消化；blockquote 转 §4）

- 视觉：热力图样式优化 + 「是否显示热力图」开关（设置项；与导航 sticky 相关——热力图关掉后导航随之上浮）。（「阅读」双入口已于 09-06 拍板保留：点时间 = 阅读弹窗。）
- 标签：平铺/树状切换按钮。
