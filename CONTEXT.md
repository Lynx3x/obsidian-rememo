...
0. **热力图批次**（2026-09-10 实施，**待 owner 目视**）：周起点设置 `HeatMapStartDay`（'sunday' 默认 / 'monday'，窗口列与星期标签随动）+ `ShowHeatMap` 开关（默认开；关闭隐藏区块，导航补 12px 分节距上浮）；分级重做 = **相对分位 4 档**（按窗口内最大值折算，原绝对阈值 1/2/4/5+ 高密度顶格问题消除）；交互反馈 = 可点格 hover 细环 + **今天 accent 描边**（修 endOf/startOf 比较死代码）+ **选中日 box-shadow 环**（修引用比较失效）；外观 = 格子 13→15px/圆角 3、星期字 10→11px、区块高 122→136。备注：`StartDate` 键确认死设置（schema 有、全代码无人消费，本批未动，待清理）。
