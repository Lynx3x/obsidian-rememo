...
0. **P3 引用系统（2026-09-05 grill 定案，ADR-0003；P3a 数据层 + P3b 聚合/评论区已交付：270374f/a770b1a/4d634a6，owner 目视全绿）**：评论 ≡ 引用统一模型——MEMO_LINK 升级（`[@标签](文件#^id)`、正文任意位置多引用、链式）+ 卡底引用条 + 父卡聚合区（折叠条+最新 3 预览，点开浮窗评论区）。**剩 P3c：输入框 @ 引用按钮**（快速建引用卡；格式已就绪）。迁移器 v2 已就绪，正式库启用待 owner 排期。
