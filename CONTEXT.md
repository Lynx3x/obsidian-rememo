...
0. **标签平铺/树状切换**（2026-09-10 实施，**待 owner 目视**）：侧栏标签标题行加双态小图标钮（平铺/树状，当前态 12% 淡底 + accent）——平铺 = 全名一行一个（新增，默认）、树状 = 层级折叠（原形态）；选择持久化 `TagListView` 设置键（默认 'flat'）。行右排布收口：**箭头在数字左侧、数字贴行右缘右对齐列**（无子行隐形占位对齐，d1cadb9）；**父标签计数 = 自身直接计数 + 全部后代子标签合计**（fillAggregateCount，随 owner 反馈实施）。顺带修尾斜杠孤儿标签（`标注/` 引用卡链接文本残留——extractTagsFromContent 过滤首尾斜杠）。新图标 view-list/tree；i18n 五语言。
