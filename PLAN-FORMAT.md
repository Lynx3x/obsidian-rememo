...
| 7 | 输入内核 | **Obsidian 原生 MarkdownEditor 嵌入 + 纯外层 DOM 控制**（~~自打包 cm6 迷你 EditorView + window capture 兜底~~、~~原生子类 buildLocalExtensions 注入~~ **定稿 2026-09-05**：内核裸编辑器不吃任何扩展注入，控制权全在外层 DOM/appendConfig，见 CONTEXT §6）；tag/file 联想 = 自产 cm6 autocomplete 经 appendConfig 注入（单份） |
...
- **P2** cm6 输入栈（host/联想/keymap/undo），旧编辑器退役——**首版已提交（b98515b/bbb87c7）但运行期键盘/联想未生效，owner 决定暂停记录交接**（症状与排查线索见 CONTEXT P2 条目下方；怀疑 Obsidian 全局 capture 吞 Mod 键）
