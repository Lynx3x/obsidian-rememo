...
## 4. memo 引用（MEMO_LINK 升级，2026-09-05 定案，论证见 [docs/adr/0003](docs/adr/0003-memo-link-reference-model.md)）

**统一模型**：评论 ≡ 引用。无「评论」实体——新评论 = 一条正文含 MEMO_LINK 指向父卡的普通 memo（自己的时间/^id/软删/回收站语义全有），渲染在被引用卡下聚合。

**语法**（正文**任意位置**，一卡可**多引用**，可**链式**引用引用卡）：
```
[@标签](日记文件名#^id)     如：[@10:15 的 memo](2026-09-05.md#^a1b2c3)
```

- 目标 = 日记文件名 + `#^id`（迁移后全卡有持久 ^id；文件重命名会断链——接受）
- 旧式 MEMO_LINK（`[@x](YYYYMMDDHHmmss+行号内存id)`）仍识别渲染，但不再产生新数据
- 识别/聚合：content 匹配目标（历史 getLinkedMemos / filter CONNECTED 同款），无需索引
- 父卡删除：引用卡独立存活，目标渲染灰「已删除」

**迁移 v2（旧评论行 → 引用卡；替换 v1 的折叠行为）**：
- 识别：memo 单位子树中的 `    - 14位时间戳 内容` 行（含 deletedAt 的已删评论丢弃计数照旧）；仅 4 空格层（顶层评论）转引用，更深缩进随其父评论转成的引用卡正文折叠
- 同文件日期（时间戳日期 = 本文件日期）→ 原地转：`- HH:mm:ss ^随机id` 头行 + 正文首行 MEMO_LINK + 内容（<br> 解码）
- 跨文件日期 → 迁移引擎升级为**跨文件操作**：目标日期日记存在则写入该文件，不存在则留父文件（不造文件）
- v1 已折叠的数据拆不出（考古证实）——dev 已迁样本用 `bak/prod-memos-20260905/` 原件重测

## 5. 认知记录（别再犯）

- **迁移器改造必须逐段对照旧版保逻辑**：2026-09-05 migrate v2 重写时整体遗漏「父行正文解码进卡」段，导致所有被迁父卡正文丢失（数据靠 bak/迁移备份找回）。改数据转换代码 = 一行为一行对照旧实现，丢失段是可迁移性最差的故障。
