...
| 2 | 卡片头行 | **纯标识**：`- 2026-09-05 12:00:00 ^id`，正文一律从缩进第二行起 |
...
| 8 | 旧数据 | 读取双模式（文件级探测），写入一律新格式；体检引擎带迁移规则，先备份后执行 |
...
- 2026-09-05 12:08:00 [deletedAt: 20260905120800123] ^d4e5f6
    （被删卡的正文保留或可清空——渲染按 isDeleted 隐藏，子树=引用暂缓）
```

解析规则：
1. 头行 = 顶层 `- ` + 时间（HH:mm(:ss) 或旧 14 位）+ 可选 `[deletedAt:…]` + 行尾 `^id`，**后面不允许再有正文文本**
2. 正文块 = 头行之后**连续 ≥4 空格缩进的行**（原样并入卡片内容，可含空行）；缩进少于 4 的非空行（新顶层 bullet / 标题 / 文件尾）结束本卡
3. 评论/引用（未来）：顶层 bullet 头行后跟 `↗日期/id` 记号 → linkId；不做进本期
4. 旧文件兼容：文件探测 = 首条 memo 头行是否"纯标识"；旧文件（头行带正文/缩进评论/<br>）走旧解析，写入端不写旧
5. 任务 `- [ ]`：任务型卡片头行 `- [ ] 时间 ^id`?—— 待 P1 PoC 定（task 前缀放头行还是正文首行，需与"纯标识"规则自洽）
...
- **P1** 新格式读取器（双模式）+ 行式渲染器 + `<br>` 兼容解码；开发库造新格式样例目视
- **P1.5** 文件级迁移规则（旧→新，备份+报告+确认），正式库 317 先只读体检再试点
